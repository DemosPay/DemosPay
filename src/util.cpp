...
const char * const BITCOIN_CONF_FILENAME = "demos.conf";
const char * const BITCOIN_PID_FILENAME = "demosd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "demos" is a composite category enabling all Demos-related debug output
            if(ptrCategory->count(string("demos"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DemosCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DemosCore
    // Mac: ~/Library/Application Support/DemosCore
    // Unix: ~/.demoscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DemosCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/DemosCore";
#else
    // Unix
    return pathRet / ".demoscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty demos.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override demos.conf
